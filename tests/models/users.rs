...
async fn can_find_by_email() {
    ...
    let existing_user =
        Model::find_by_email(&boot.app_context.db, "user1@example.com").await;
...
async fn can_find_by_pid() {
    ...
    let existing_user = Model::find_by_id_str(
        &boot.app_context.db,
        "11111111-1111-1111-1111-111111111111",
    )
    .await;
    let non_existing_user_results = Model::find_by_id_str(
        &boot.app_context.db,
...
async fn magic_link() {
    ...
    let updated_user = Model::find_by_id_str(
        &boot.app_context.db,
        "11111111-1111-1111-1111-111111111111",
    )
    .await
    ...
    let now = Local::now();
    let should_expired_at =
        now + Duration::minutes(users::MAGIC_LINK_EXPIRATION_MIN.into());
